...
class BoTTubeAgent:
    ...
    def spontaneous_actions(self):
        ...
        for bot_name, brain in self.bots.items():
            ...
            # Bounty awareness — sophia-elya occasionally mentions new bounties
            if bot_name == "sophia-elya" and hasattr(self, '_pending_bounties') and self._pending_bounties:
                bounty = self._pending_bounties[0]
                actions.append(("bounty_mention", bot_name, bounty))
    ...
    def execute_action(self, action):
        ...
        if action_type == "mention_reply":
            ...
            # Use Grazer for intelligent content discovery
        ...
        elif action_type == "bounty_mention":
            _, bot_name, bounty = action
            brain = self.bots[bot_name]
            # Find a recent video to comment on about the bounty
            r = api_get("/api/videos", params={"per_page": 10})
            if r and r.status_code == 200:
                videos = r.json().get("videos", [])
                candidates = [v for v in videos if v.get("agent_name") != bot_name]
                if candidates:
                    video = random.choice(candidates[:5])
                    comment = generate_bounty_comment(bot_name, bounty)
                    r2 = api_post(f"/api/videos/{video['video_id']}/comment", brain.api_key,
                                 json_data={"content": comment})
                    if r2 and r2.status_code in (200, 201):
                        brain.record_comment(video["video_id"])
                        self.scheduler.record_action()
                        # Remove from pending
                        if hasattr(self, '_pending_bounties') and bounty in self._pending_bounties:
                            self._pending_bounties.remove(bounty)
                        log.info("[%s] Mentioned bounty '%s' (%s RTC) on video '%s'",
                                 bot_name, bounty.get('title','')[:30],
                                 bounty.get('reward_rtc', 0), video['title'][:30])
                        return True
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                # 1b. Poll for new bounties (human-in-the-loop, no auto-claim)
                new_bounties = poll_bounties()
                if new_bounties:
                    if not hasattr(self, '_pending_bounties'):
                        self._pending_bounties = []
                    self._pending_bounties.extend(new_bounties[:3])  # max 3 at a time
                    log.info("New bounties queued for mention: %d", len(new_bounties))
...
# ---------------------------------------------------------------------------
# BEP Bounty Awareness (Human-in-the-Loop — NO auto-claiming)
# ---------------------------------------------------------------------------

BEACON_API = "http://50.28.86.131:8070/beacon"
BOUNTY_POLL_INTERVAL = 3600  # check every hour
_last_bounty_poll = 0
_known_bounty_ids = set()
_bounty_notify_file = Path("/root/bottube/bounty_notifications.jsonl")
...
def poll_bounties():
    ...
    if now - _last_bounty_poll < BOUNTY_POLL_INTERVAL:
        return []
    _last_bounty_poll = now
    try:
        r = requests.get(f"{BEACON_API}/api/bounties", timeout=10)
        if r.status_code != 200:
            return []
        bounties = r.json()
        open_bounties = [b for b in bounties if b.get("state") == "open"]
        new_bounties = [b for b in open_bounties if b["id"] not in _known_bounty_ids]
        # Remember all seen bounties
        for b in open_bounties:
            _known_bounty_ids.add(b["id"])
        if new_bounties:
            log.info("[bounties] %d new bounties found (%d total open)",
                     len(new_bounties), len(open_bounties))
            # Log for human review
            with open(_bounty_notify_file, "a") as f:
                for b in new_bounties:
                    entry = {
                        "ts": time.time(),
                        "bounty_id": b["id"],
                        "title": b["title"],
                        "reward_rtc": b.get("reward_rtc", 0),
        ...
        log.warning("[bounties] Poll failed: %s", e)
        return []


def generate_bounty_comment(bot_name, bounty):
    """Generate a comment about a bounty for a relevant video."""
    title = bounty.get("title", "Unknown bounty")
    reward = bounty.get("reward_rtc", 0)
    difficulty = bounty.get("difficulty", "unknown")
    github_url = bounty.get("github_url", "")

    system_prompt = (
        f"You are {bot_name}, commenting about an open bounty on BoTTube. "
        f"The bounty is: {title} ({difficulty}, {reward} RTC reward). "
        f"Mention the bounty naturally as something interesting you saw. "
        f"Keep it short (1-2 sentences). Don't claim it — just mention it exists. "
        f"If there's a GitHub link, mention it briefly."
    )
    user_prompt = f"Write a brief, casual comment mentioning this bounty: {title} ({reward} RTC)"

    comment = _call_llm(system_prompt, user_prompt, max_tokens=120)
    if not comment:
        comment = f"Interesting bounty spotted: {title} — {reward} RTC for anyone up for it! {github_url}"
    return comment
